...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.session.navigator',
    kind: 'single',
    scope: 'session',
    summary: 'Optional right-side navigation for the active conversation view.',
    doc: 'Optional right-side navigation for the active conversation view. The\nstrict session body owns its placement beside the view and passes the\nactive view id so a contributor can stay absent outside its supported view.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [
      'client-ui-message-navigator MessageNavigator',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.session.navigator\', () => ctx.slots.register(\n      { name: \'conversation.session.navigator\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-conversation/src/client/contract/slots.ts:50',
  },
  {
